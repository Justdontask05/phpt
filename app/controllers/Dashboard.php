...
class Dashboard extends Controller {
    ...
    public function index() {
        ...
        $biolinks_clicks_total = database()->query("SELECT SUM(`clicks`) AS `total` FROM `links` WHERE `user_id` = {$this->user->user_id} AND `type` = 'biolink'")->fetch_object()->total;
        ...
        $data = [
            ...
            'biolinks_total'        => $biolinks_total,
            'links_clicks_total'    => $links_clicks_total,
            'biolinks_clicks_total' => $biolinks_clicks_total
        ];
