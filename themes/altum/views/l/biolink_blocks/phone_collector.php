...
<div data-biolink-block-id="<?= $data->link->biolink_block_id ?>" class="col-12 my-2">
    <a href="#" data-toggle="modal" data-target="#phone_collector_<?= $data->link->link_id ?>" class="btn btn-block btn-primary link-btn link-hover-animation <?= 'link-btn-' . $data->link->settings->border_radius ?> <?= $data->link->design->link_class ?>" style="<?= $data->link->design->link_style ?>">
...
<?php ob_start() ?>
<div class="modal fade" id="phone_collector_<?= $data->link->link_id ?>" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    <script>
        ...
        $('form[id^="phone_collector_"]').on('submit', event => {
            ...
            if(!is_converted) {
                ...
                $.ajax({
                    type: 'POST',
                    url: `${base_url}l/link/phone_collector`,
