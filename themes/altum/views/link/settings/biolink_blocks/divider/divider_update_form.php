...
<form name="update_biolink_" method="post" role="form">
    ...
    <div <?= $this->user->plan_settings->custom_colored_links ? null : 'data-toggle="tooltip" title="' . language()->global->info_message->plan_feature_no_access . '"' ?>>
        <div class="<?= $this->user->plan_settings->custom_colored_links ? null : 'container-disabled' ?>">
            ...
            <div class="form-group">
                <label for="<?= 'divider_background_color' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= language()->create_biolink_link_modal->input->background_color ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'divider_icon' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-globe fa-sm text-muted mr-1"></i> <?= language()->create_biolink_link_modal->input->icon ?></label>
