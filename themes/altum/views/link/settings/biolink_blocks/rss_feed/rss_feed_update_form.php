...
<form name="update_biolink_" method="post" role="form">
    ...
    <div class="form-group">
        <label for="<?= 'rss_feed_location_url_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= language()->create_biolink_rss_feed_modal->location_url ?></label>
        <input id="<?= 'rss_feed_location_url_' . $row->biolink_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" placeholder="<?= language()->create_biolink_rss_feed_modal->location_url_placeholder ?>" required="required" />
    </div>
    ...
    <div <?= $this->user->plan_settings->custom_colored_links ? null : 'data-toggle="tooltip" title="' . language()->global->info_message->plan_feature_no_access . '"' ?>>
        <div class="<?= $this->user->plan_settings->custom_colored_links ? null : 'container-disabled' ?>">
            <div class="form-group">
                <label for="<?= 'rss_feed_text_color_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= language()->create_biolink_link_modal->input->text_color ?></label>
            ...
            <div class="form-group">
                <label for="<?= 'rss_feed_background_color_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= language()->create_biolink_link_modal->input->background_color ?></label>
            ...
            <div class="form-group">
                <label for="<?= 'rss_border_width_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-border-style fa-sm text-muted mr-1"></i> <?= language()->create_biolink_link_modal->input->border_width ?></label>
            ...
            <div class="form-group">
                <label for="<?= 'rss_border_color_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= language()->create_biolink_link_modal->input->border_color ?></label>
