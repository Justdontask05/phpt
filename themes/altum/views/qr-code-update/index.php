                        ...
                        <div class="form-group">
                            ...
                            <select id="type" name="type" class="form-control">
                                <?php foreach(array_keys($data->qr_code_settings['type']) as $type): ?>
                                    <option value="<?= $type ?>" <?= $data->qr_code->type == $type ? 'selected="selected"' : null ?>><?= language()->qr_codes->type->{$type} ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                            ...
                            <div class="form-group" data-type="paypal">
                                ...
                                <select id="paypal_type" name="paypal_type" class="form-control" data-reload-qr-code>
                                    <?php foreach($data->qr_code_settings['type']['paypal']['type'] as $key => $value): ?>
                                        <option value="<?= $key ?>" <?= ($data->qr_code->settings->paypal_type ?? null) == $key ? 'selected="selected"' : null?>><?= language()->qr_codes->input->{'paypal_type_' . $key} ?></option>
                                    <?php endforeach ?>
                                </select>
                            </div>
            ...
            <div class="row mb-4 d-print-none">
                ...
                <div class="col-12 col-lg-6 mb-3 mb-lg-0 dropdown">
                    ...
                    <div class="dropdown-menu">
                        <a href="<?= UPLOADS_FULL_URL . 'qr_code/' . $data->qr_code->qr_code ?>" id="download_svg" class="dropdown-item" download="<?= settings()->main->title . '.svg' ?>"><?= language()->qr_codes->download_svg ?></a>
                        <button type="button" class="dropdown-item" onclick="convert_svg_to_others(null, 'png', '<?= $data->qr_code->name . '.png' ?>');"><?= language()->qr_codes->download_png ?></button>
                        <button type="button" class="dropdown-item" onclick="convert_svg_to_others(null, 'jpg', '<?= $data->qr_code->name . '.jpg' ?>');"><?= language()->qr_codes->download_jpg ?></button>
                        <button type="button" class="dropdown-item" onclick="convert_svg_to_others(null, 'webp', '<?= $data->qr_code->name . '.webp' ?>');"><?= language()->qr_codes->download_webp ?></button>
                    </div>
                </div>
            </div>
